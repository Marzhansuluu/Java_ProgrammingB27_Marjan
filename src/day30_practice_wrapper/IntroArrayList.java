package day30_practice_wrapper;

import java.util.ArrayList;
import java.util.Arrays;

public class IntroArrayList {
    public static void main(String[] args) {

        int [] a = {4,2,1};
        System.out.println("Number of element :" + a.length);
        System.out.println("Print the whole array: " + Arrays.toString(a));
        System.out.println("Access the firsl element: " + a[0]);
        System.out.println("Add elements to the array : CANNOT change size of array after creation");
        System.out.println("Remove element from the array -CANNOT do it");
        System.out.println("Array can hold values that are primitive types and object types");

        System.out.println("-------------------------------------------------");

        ArrayList<Integer> nums = new ArrayList<>();// empty ArrayList
        System.out.println("Number of elements: " + nums.size());
        System.out.println("Print whole ArrayList: "+ nums);

        System.out.println("Adding arrayList: .add() method");
        nums.add(5);
        nums.add(10);
        System.out.println("After adding number to array: " + nums);//[5,10]
        System.out.println(nums.size());//2

    }
}
